...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    try {
        ...
        const [electionArticles, dailyBriefs, factChecks] = await Promise.all([
            ElectionArticle.find({ status: 'published' }).select('slug updatedAt').lean(),
        ...
        return [...staticPages, ...articlePages, ...leaderPages, ...electionArticlePages, ...dailyBriefPages, ...factCheckPages];
    } catch (error) {
